...
NETWORK_DROPIN_DIR="/etc/systemd/system/atty-guard.service.d"
NETWORK_DROPIN_FILE="$NETWORK_DROPIN_DIR/network.conf"
...
if [[ $WITH_NETWORK -eq 1 || ($NEEDS_NETWORK -eq 1 && $WITHOUT_NETWORK -ne 1) ]]; then
    install -d -o root -g root -m 0755 "$NETWORK_DROPIN_DIR"
...
EOF
    ...
    if [[ -f "$NETWORK_DROPIN_FILE" ]]; then
        ...
        rmdir "$NETWORK_DROPIN_DIR" 2>/dev/null || true
    else
