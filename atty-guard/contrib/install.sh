...
if [[ $WITH_EBPF -eq 1 ]]; then
    ...
    # line; if `ebpf` isn't there, the drop-in's --enable-ebpf
    # would just error at daemon startup. Catch the misconfig now,
    # before systemctl enable surfaces it as a confusing red unit.
