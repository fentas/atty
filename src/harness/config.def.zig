//! harness configuration — committed template (dwm `config.def.h` style).
//!
...
//! the Suckless-extensible surface: to change the composed harness, edit the
//! `modules` tuple and recompile. No runtime plugin loader, no flags — the
//! binary IS the configured harness.
//!
...
/// The composed harness — observers + fault injectors, comptime-walked by
...
pub const modules = .{
    ...
    cast_recorder(.{ .path = "harness-run.cast" }),
