//! harness — the configured TTY-test binary: the "single binary precompiled
//! with config.zig" form of the framework (docs/harness.md).
//!
//! This example drives `bash` through the harness and asserts on the RENDERED
...
pub fn main() !void {
    ...
    try h.send("printf 'hello-from-harness\\n'\r");
    try check("command output rendered", try h.waitFor("hello-from-harness", 3000));
...
fn check(label: []const u8, ok: bool) !void {
    if (!ok) {
        std.debug.print("harness example: FAIL — {s}\n", .{label});
