...
pub fn Module(comptime cfg: types.Config) type {
    return struct {
        ...
        /// stacks the same way so dialog and auto share the user's
        /// domain context. Chat surfaces dispatch as `.dialog`/
        /// `.auto` so they don't need a separate effective prompt.
