...
pub const StatusBar = struct {
    ...
    /// if the text matches the last paint we emit zero bytes.
    pub fn render(self: *StatusBar, w: *std.Io.Writer) std.Io.Writer.Error!void {
        if (self.last_valid and self.last_len == self.text_len and
            std.mem.eql(u8, self.last_buf[0..self.last_len], self.text_buf[0..self.text_len]))
            return;

        try w.writeAll(ansi.save_cursor);
        try w.print("\x1B[{d};1H\x1B[K", .{self.rows}); // CUP last row, erase
        try w.print("{f}", .{self.style});
        try w.writeAll(self.text_buf[0..self.text_len]);
        try w.writeAll(ansi.sgr_reset);
        try w.writeAll(ansi.restore_cursor);

        @memcpy(self.last_buf[0..self.text_len], self.text_buf[0..self.text_len]);
        self.last_len = self.text_len;
