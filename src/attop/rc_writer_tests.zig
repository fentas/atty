...
test "buildBlock embeds the init path between the markers" {
    const b = try rc_writer.buildBlock(testing.allocator, "/home/u/.config/atty/init.bash");
...
test "upsertBlock appends to an existing rc, preserving it" {
    const rc = "export PS1='$ '\nalias ll='ls -l'\n";
    const out = try rc_writer.upsertBlock(testing.allocator, rc, "/p/init.bash");
...
test "upsertBlock is idempotent (apply twice == apply once)" {
    const rc = "line1\nline2\n";
    const once = try rc_writer.upsertBlock(testing.allocator, rc, "/p/init.zsh");
    defer testing.allocator.free(once);
    const twice = try rc_writer.upsertBlock(testing.allocator, once, "/p/init.zsh");
...
test "upsertBlock rewrites a changed init path in place, preserving surroundings" {
    const rc = "before\n";
    const v1 = try rc_writer.upsertBlock(testing.allocator, rc, "/old/init.bash");
    ...
    const v2 = try rc_writer.upsertBlock(testing.allocator, with_tail, "/new/init.zsh");
...
test "upsertBlock on an empty rc yields just the block" {
    const out = try rc_writer.upsertBlock(testing.allocator, "", "/p/init.bash");
    defer testing.allocator.free(out);
