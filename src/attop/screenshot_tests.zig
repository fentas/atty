...
test "Setup renders the checklist across widths (up, neutral, down)" {
    ...
    for (widths) |cols| {
        ...
        const neut = try screen(testing.allocator, setup.renderSetup(&buf, neutral_m, .{ .atty_on_path = true, .under_atty = true, .shell_integrated = true }, cols, 40), 40, cols);
        ...
        const down = try screen(testing.allocator, setup.renderSetup(&buf, null, .{ .atty_on_path = true, .under_atty = false, .shell_integrated = true }, cols, 40), 40, cols);
        defer testing.allocator.free(down);
        try expectOnScreen(down, &.{ "not reachable", "sudo systemctl start atty-guard", "unknown (daemon down)", "not under atty" });

        // The wizard state: atty not installed + shell not wired → the fixes.
        const noatty = try screen(testing.allocator, setup.renderSetup(&buf, null, .{ .atty_on_path = false, .under_atty = false, .shell_integrated = false }, cols, 40), 40, cols);
        defer testing.allocator.free(noatty);
        try expectOnScreen(noatty, &.{ "not installed", "install: curl -fsSL https://bin.atty.sh | sh", "not wired", "atty init bash" });
    }
}
