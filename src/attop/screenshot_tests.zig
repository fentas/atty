...
test "Setup renders the checklist across widths (up, neutral, down)" {
    ...
    for (widths) |cols| {
        const up = try screen(testing.allocator, setup.renderSetup(&buf, homeMetrics("strict"), true, true, true, cols, 40), 40, cols);
        ...
        const neut = try screen(testing.allocator, setup.renderSetup(&buf, neutral_m, true, true, true, cols, 40), 40, cols);
        ...
        const down = try screen(testing.allocator, setup.renderSetup(&buf, null, true, false, true, cols, 40), 40, cols);
        defer testing.allocator.free(down);
        ...
        const noatty = try screen(testing.allocator, setup.renderSetup(&buf, null, false, false, false, cols, 40), 40, cols);
