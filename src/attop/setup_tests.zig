...
test "atty installed → ✓; not on PATH → ✗ + the install fix" {
    ...
    const yes = setup.renderSetup(&buf, m, true, true, true, 120, 40); // atty_on_path = true
    ...
    const no = setup.renderSetup(&buf2, m, false, true, true, 120, 40); // not on PATH
...
test "setup surfaces the enforcement depth" {
    ...
    const out = setup.renderSetup(&buf, m, true, true, true, 120, 40);
...
test "setup shell row → wired ✓; not wired → ✗ + the wire fix" {
    ...
    const wired = setup.renderSetup(&buf, m, true, true, true, 120, 40); // shell_integrated = true
    ...
    var buf2: [4096]u8 = undefined;
    const not = setup.renderSetup(&buf2, m, true, true, false, 120, 40); // not wired
...
test "daemon down → bad mark + fix; session still checkable" {
    var buf: [4096]u8 = undefined;
    const out = setup.renderSetup(&buf, null, true, false, true, 120, 40);
...
test "eBPF: daemon \"off\" keeps the install fix; future status verbatim; empty → unknown" {
    ...
    const out = setup.renderSetup(&buf, off, true, true, true, 120, 40);
    ...
    const out2 = setup.renderSetup(&buf2, future, true, true, true, 120, 40);
    ...
    const out3 = setup.renderSetup(&buf3, absent, true, true, true, 120, 40);
