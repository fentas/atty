...
pub const modules = .{
    ...
    atty.modules.llm.configure(.{
        ...
        .fixture_responses = &.{
            ...
            // OSC-133-integrated. Zig's `\\` multi-line literal
            // passes the bytes through verbatim — `\033` is literal
            // backslash-zero-three-three; bash + printf then
            // interpret it as ESC at execution time. During the
            // shell echo of the typed line, atty sees literal
            // characters (no raw ESC), so the markers only fire
            // after printf runs.
